...
def add_actor_information_and_train(
    ...
    while True:
        ...
        def _sample_hilserl_batch() -> dict:
            ...
            # Default: 50/50 online vs human (offline+interventions). When available, also include
            # a small auxiliary batch from the success-tail buffer so sparse rewards appear in most batches.
            success_batch_size = 0
            human_batch_size = offline_batch_size
            if success_replay_buffer is not None:
                try:
                    if len(success_replay_buffer) > 0:
                        success_batch_size = max(1, offline_batch_size // 2)
                        success_batch_size = min(success_batch_size, offline_batch_size)
                        human_batch_size = offline_batch_size - success_batch_size
                        if human_batch_size == 0 and offline_batch_size > 1:
                            human_batch_size = 1
                            success_batch_size = offline_batch_size - human_batch_size
                except Exception:
                    success_batch_size = 0
                    human_batch_size = offline_batch_size

            if success_batch_size > 0 and success_replay_buffer is not None:
                try:
                    batch_success = success_replay_buffer.sample(success_batch_size)
                    ...
                    additions.append(batch_success)
                except Exception as exc:
                    if not warned_missing_success_batch:
                        logging.info(
                            "[LEARNER] Success buffer not ready (%s); continuing without it.",
                            exc,
                        )
            ...
            if human_batch_size > 0:
                try:
                    ...
                    additions.append(batch_human)
                except Exception as exc:
                    if not warned_missing_human_batch:
                        logging.info(
                            "[LEARNER] Human buffer not ready (%s); falling back to online sampling for now.",
                            exc,
                        )
                        warned_missing_human_batch = True
                    batch_fallback = (
                        next(online_iterator)
                        if human_batch_size == online_batch_size
                        else replay_buffer.sample(human_batch_size)
                    )
                    additions.append(batch_fallback)

            for extra in additions:
                batch = concatenate_batch_transitions(
                    left_batch_transitions=batch,
                    right_batch_transition=extra,
                )
            return batch
        ...
        actor_bc_only = False
        require_human = bool(getattr(cfg.policy, "require_human_buffer_for_actor_update", False))
        if require_human:
            try:
                human_size = len(offline_replay_buffer) if offline_replay_buffer is not None else 0
            except Exception:
                human_size = 0
            if human_size <= 0:
                actor_updates_allowed = False
                training_infos["actor_updates_skipped_no_human_buffer"] = 1
        ...
        has_intervention_in_batch = False
        is_intervention_t: torch.Tensor | None = None
        try:
            comp_info = forward_batch.get("complementary_info")
            if isinstance(comp_info, dict):
                is_intervention = comp_info.get(TeleopEvents.IS_INTERVENTION.value)
                if is_intervention is None:
                    is_intervention = comp_info.get(TeleopEvents.IS_INTERVENTION)
                if isinstance(is_intervention, torch.Tensor):
                    is_intervention_t = is_intervention.view(-1)
                    has_intervention_in_batch = bool((is_intervention_t > 0.5).any().item())
                else:
                    has_intervention_in_batch = bool(is_intervention)
        except Exception:
            has_intervention_in_batch = False
            is_intervention_t = None
        if min_positive > 0:
            try:
            ...
            # If we have intervention samples, allow a BC-only actor update even when the reward signal
            # is still absent (this bootstraps behavior in HIL-SERL before any success is observed).
            if not actor_updates_allowed and has_intervention_in_batch:
                actor_updates_allowed = True
                actor_bc_only = True
                training_infos["actor_updates_bc_only"] = 1
                if skip_no_positive_in_batch:
                    training_infos["actor_updates_bc_only_no_positive_in_batch"] = 1
                if skip_insufficient_positive_transitions:
                    training_infos["actor_updates_bc_only_insufficient_positive_reward_transitions"] = 1
        ...
        if actor_updates_allowed and optimization_step % policy_update_freq == 0:
            ...
            actor_batch: dict = forward_batch

            if actor_bc_only:
                actor_batch = dict(actor_batch)
                actor_batch["actor_bc_only"] = True
            actor_output = policy.forward(actor_batch, model="actor")
            ...
            bc_loss = getattr(policy, "_last_intervention_bc_loss", None)
            if isinstance(bc_loss, torch.Tensor):
                training_infos["intervention_bc_loss"] = float(bc_loss.item())

            if not actor_bc_only:
                ...
                training_infos["temperature_updates_skipped_bc_only"] = 1
