...
class TimeLimitProcessorStep(TruncatedProcessorStep):
    ...
    def truncated(self, truncated: bool) -> bool:
        ...
        if self.current_step >= self.max_episode_steps:
            truncated = True
        # Explicitly preserve the incoming truncated value when step limit is not reached
        # (no else clause needed as we want to keep the incoming value)
        return truncated
...
class GripperPenaltyProcessorStep(ProcessorStep):
    ...
    def __call__(self, transition: EnvTransition) -> EnvTransition:
        ...
        # Validate action has at least one element before accessing
