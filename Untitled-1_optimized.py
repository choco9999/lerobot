...
# 0) ライブラリのインポート
# ------------------
import os, re, ast, math, time, hashlib, tempfile, pickle, warnings
...
def compute_text_list_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["Instruction_char_len"] = df["RecipeInstructions"].apply(lambda x: sum(len(str(s)) for s in ensure_list(x))).astype(np.int32)
    df["Instruction_word_count"] = df["RecipeInstructions"].apply(lambda x: sum(len(str(s).split()) for s in ensure_list(x))).astype(np.int32)
    ...
    df["Description_word_count"] = df["Description"].fillna("").apply(lambda x: len(str(x).split())).astype(np.int32)
...
NO_QUANT_TERMS = set(map(str.lower, """
parsley cilantro basil mint chives dill oregano thyme rosemary sage coriander
coriander leaves coriander leaf scallion scallions green onion green onions spring onion spring onions
salt pepper black pepper white pepper paprika chili powder cayenne cumin turmeric cinnamon nutmeg allspice clove cloves
garnish lemon zest lemon rind lemon peel orange zest orange rind orange peel zest rind peel
""".split()))
NO_QUANT_REGEXES = [
...
def is_noquant_candidate(part: str) -> bool:
    p = part.lower().strip()
    if p in NO_QUANT_TERMS: return True
    for rx in NO_QUANT_REGEXES:
        if rx.search(p): return True
    if "lemon" in p and re.search(r"\b(zest|rind|peel)\b", p): return True
    if "juice" in p: return False
    return False
...
def compress_extra_quants(quants, target_len):
    if len(quants) <= target_len: return quants
...
def smart_align_row(parts, quants):
    ...
    if len(parts) == 0 and len(quants) > 0: return [], []
    if len(quants) > len(parts): quants = compress_extra_quants(quants, len(parts))
    if len(parts) > len(quants):
        needed = len(parts) - len(quants)
        zero_pos = choose_0_positions(parts, needed)
        aligned, j = [], 0
        for i in range(len(parts)):
            if i in zero_pos: aligned.append(0)
            else:
...
def clean_lists(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    df["RecipeIngredientParts"] = df["RecipeIngredientParts"].apply(to_list).apply(lambda L: [str(x).strip() for x in L if str(x).strip()!=""])
    df["RecipeIngredientQuantities"] = df["RecipeIngredientQuantities"].apply(to_list).apply(
        lambda L: [clean_quantity_token(str(x)) for x in L if str(x).strip()!=""]
    )
    return df

def process_df_smart(df: pd.DataFrame) -> pd.DataFrame:
    df = clean_lists(df)
    fixed = df.apply(lambda r: smart_align_row(r["RecipeIngredientParts"], r["RecipeIngredientQuantities"]), axis=1)
...
def convert_quantities(df):
    df = df.copy()
    df["RecipeIngredientQuantities"] = df["RecipeIngredientQuantities"].apply(lambda L: [quantity_to_float(x) for x in L])
    return df
...
def build_ingredient_features(df):
    ...
    df["IngredientParts_List"] = df["RecipeIngredientParts"].apply(lambda parts: [normalize_part(p) for p in parts if str(p).strip()])
    ...
    def pair_text(parts, quants):
        ...
        return " ".join(pairs)
    df["IngredientPairText"] = [pair_text(p, q) for p, q in zip(df["IngredientParts_List"], df["IngredientQuantities_List"])]
    ...
    df["Ingredient_name_char_len"]     = df["IngredientParts_List"].apply(lambda p: sum(len(s) for s in p)).astype(np.int32)
    df["Ingredient_name_word_count"]   = df["IngredientParts_List"].apply(lambda p: sum(len(s.split()) for s in p)).astype(np.int16)
    return df
...
counter = Counter()
for S in df_train["IngredientParts_List"].apply(set): counter.update(S)
for S in df_test["IngredientParts_List"].apply(set):  counter.update(S)
...
def add_ingredient_ratio_features(df):
    ...
    df["Ingredient_unique_ratio"]    = safe_ratio(df["Ingredient_unique_count"].astype(float), df["IngredientQty_count"].astype(float))
    df["Ingredient_density_per_min"] = safe_ratio(df["IngredientQty_count"].astype(float), df["TotalTime"].astype(float))
    df["Ingredient_char_len_per_part"] = safe_ratio(df["Ingredient_name_char_len"].astype(float), df["IngredientQty_count"].astype(float))
    # log transform
...
def cache_all_images(df, output_dir="/tmp/images", max_imgs=1, img_size=128, timeout=4):
    ...
    def resolve_urls(val):
        if isinstance(val, list): return val[:max_imgs]
        if isinstance(val, str):
            if val.startswith("["):
                try:
                    L = eval(val)
                    if isinstance(L, list): return L[:max_imgs]
                except Exception: return [val]
            return [val]
        return []

    urls = []
    for v in df["Images"]: urls.extend(resolve_urls(v))
...
class LoRALinear(nn.Module):
    ...
    def forward(self,x): return self.base(x)+self.B(self.dropout(self.A(x)))*self.scale
    ...
    def _inject(self,module,r,alpha,dropout):
        for name,child in module.named_children():
            if isinstance(child,nn.Linear):
                ...
                if child.bias is not None: new.base.bias.data.copy_(child.bias)
                setattr(module,name,new)
            else: self._inject(child,r,alpha,dropout)
    def forward(self,x):
        f=self.backbone(x)
...
class LocalImageDataset(Dataset):
    def __init__(self, df, transform, labels=True, img_size=128, cache_dir="/tmp/images"):
        self.df=df.copy(); self.transform=transform; self.labels=labels
        self.img_size=img_size; self.cache_dir=cache_dir
...
def collate_variable_images(batch):
    if isinstance(batch[0],(list,tuple)) and len(batch[0])==3:
        ...
        for x,y1,y2 in batch:
            xs.append(x.unsqueeze(0)); yc.append(y1); yr.append(y2)
        return torch.cat(xs),torch.stack(yc),torch.stack(yr)
    else:
        xs=[x.unsqueeze(0) for x in batch]; return torch.cat(xs)


# ============================================================
# 4) build_lora_features_fast_url（キャッシュ時リサイズ対応）
# ============================================================
def build_lora_features_fast_url(df_train, df_test,
                                 epochs=50,batch_size=2,lr=1e-4,
                                 precision="16-mixed",patience=3,
                                 cache_dir="/tmp/images",img_size=128):
    ...
    val_frac=0.1; n_val=max(1,int(len(df_train)*val_frac))
    df_val=df_train.sample(n_val,random_state=42); df_trn=df_train.drop(df_val.index)

    train_dl=DataLoader(LocalImageDataset(df_trn,transform,True,cache_dir),
                        batch_size=batch_size,shuffle=True,num_workers=2,
                        pin_memory=False,collate_fn=collate_variable_images)
    val_dl=DataLoader(LocalImageDataset(df_val,transform,True,cache_dir),
                      batch_size=batch_size,shuffle=False,num_workers=2,
                      pin_memory=False,collate_fn=collate_variable_images)
    test_dl=DataLoader(LocalImageDataset(df_test,transform,False,cache_dir),
                       batch_size=batch_size,shuffle=False,num_workers=2,
                       pin_memory=False,collate_fn=collate_variable_images)
    ...
    for epoch in range(1,epochs+1):
        ...
        for xb,yc,yr in tqdm(train_dl,desc=f"[Train]{epoch}/{epochs}"):
            ...
            B,N,C,H,W=xb.shape; xb_flat=xb.view(B*N,C,H,W)
            ...
            total+=loss.item()
            del xb,yc,yr,xb_flat,pc,pr,loss; torch.cuda.empty_cache()
        # validation
        ...
        if val_loss<best_loss-1e-4:
            best_loss=val_loss; best_state={k:v.cpu() for k,v in model.state_dict().items()}; no_imp=0
        else:
            no_imp+=1
            if no_imp>=patience:
                print(f"⏹️ Early stop at {epoch}, best val={best_loss:.4f}"); break
    if best_state is not None: model.load_state_dict(best_state)

    @torch.no_grad()
    def extract(df,loader):
        model.eval(); out=np.zeros((len(df),8),dtype=np.float32); ptr=0
        for batch in tqdm(loader,desc="[Extract]"):
            xb=batch[0] if isinstance(batch,(list,tuple)) else batch; xb=xb.to(device)
            if xb.ndim==5:B,N,C,H,W=xb.shape; xb_flat=xb.view(B*N,C,H,W)
            else:B,C,H,W=xb.shape; xb_flat=xb; N=1
            with torch.cuda.amp.autocast(enabled=("16" in precision)):
                cal,rat=model(xb_flat)
            cal=cal.view(B,N); rat=rat.view(B,N)
            feats=torch.stack([cal.mean(1),cal.std(1),cal.max(1).values,cal.min(1).values,
                               rat.mean(1),rat.std(1),rat.max(1).values,rat.min(1).values],dim=1).cpu().numpy()
            out[ptr:ptr+B]=feats; ptr+=B
            del xb,xb_flat,cal,rat,feats; torch.cuda.empty_cache()
    ...
    tr_feats=extract(df_train,DataLoader(LocalImageDataset(df_train,transform,True,cache_dir),
                                         batch_size=batch_size,shuffle=False,num_workers=2,
                                         pin_memory=False,collate_fn=collate_variable_images))
...
df_review_train_fe = pd.concat([
    df_review_train.reset_index(drop=True),
    pd.DataFrame(Xtr_svd, columns=tfidf_cols)
], axis=1)
df_review_test_fe = pd.concat([
    df_review_test.reset_index(drop=True),
    pd.DataFrame(Xte_svd, columns=tfidf_cols)
], axis=1)
...
ora_oof_tr, lora_te = build_lora_features_fast_url(
    ...
    epochs=50, batch_size=2, lr=1e-4,
    precision="16-mixed", patience=3,
    cache_dir="/tmp/images", img_size=128  # 🔽 ←ここでキャッシュ時のリサイズ指定
)
...
def join_list_column(col):
    def convert(v):
        ...
        if isinstance(v, str) and v.startswith("["):
            try:
                p = ast.literal_eval(v)
                if isinstance(p, list): return " ".join(map(str, p))
            except Exception:
                return v
        if pd.isna(v): return ""
        return str(v)
...
def tfidf_svd(train_series, test_series, prefix,
              n_components=50, max_features=10000, min_df=3):
    """
...
text_columns = ["Description","RecipeInstructions","IngredientsText","IngredientPairText","NameText","RecipeCategoryText"]
...
def tfidf_block(colname, n_components, max_features, min_df):
    tr, te = tfidf_svd(df_train[colname], df_test[colname], colname, n_components=n_components, max_features=max_features, min_df=min_df)
    return tr, te
...
if USE_GROUP_KFOLD and ("RecipeId" in X_train.columns):
    ...
    split_iter = lambda y, bins: StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=RANDOM_SEED).split(X_train, bins)
