...
By default, SARM starts from the public pretrained checkpoint
`lerobot-data-collection/level2_sarm9`. To train fully from scratch, set
...
### Fine-tune from a Pretrained SARM

To continue training from a SARM model already on the Hugging Face Hub (or from a local checkpoint), pass
`--policy.path`. The loaded policy config is reused, and CLI args override only what you set.

```bash
lerobot-train \
  --policy.path=your-username/your-pretrained-sarm \
  --dataset.repo_id=your-username/your-new-dataset \
  --output_dir=outputs/train/sarm_finetune \
  --batch_size=32 \
  --steps=5000 \
  --wandb.enable=true \
  --wandb.project=sarm
...
If you want to override mode-specific settings while fine-tuning, add them explicitly (for example
`--policy.annotation_mode=dual` and `--policy.image_key=observation.images.base`).
...
| `--policy.pretrained_path` | Hub model or local checkpoint used to initialize SARM              | `lerobot-data-collection/level2_sarm9` |
...
For `policy.type=pi05`, LeRobot defaults to the pretrained base checkpoint `lerobot/pi05_base` when `use_rabc=true` and `policy.pretrained_path` is omitted. Pass `--policy.pretrained_path=null` to train from scratch, or set another checkpoint explicitly:
